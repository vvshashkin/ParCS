...
subroutine set_NHlin_initial_conditions(stvec, namelist_str, params, &
    ...
    else if(test_case_num == 3) then
        call set_NHlin_sound_wave(stvec, test_case_num, params%ts, params%te,  &
                                    params%mesh, params%radx, params%nz, params%zh, params%z)
    else
...
subroutine set_NHlin_sound_wave(stvec, test_case_num, ts, te, mesh, radx, nz, zh, z)
...
!                    stvec%theta(ind)%p(i,j,k) = theta_max*r0**2/(r0**2+dist**2) * &
!                                                sin(2._8*pi*zh(k)/Lz)
!                    stvec%theta(ind)%p(i,j,k) = theta_max * &
!                                                sin(2._8*pi*zh(k)/Lz)
                    stvec%prex(ind)%p(i,j,k) = 0.01 * &
                                                sin(2._8*pi*z(k)/Lz)
...
end subroutine set_NHlin_sound_wave
