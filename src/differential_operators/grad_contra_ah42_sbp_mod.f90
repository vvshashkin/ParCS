module grad_contra_ah42_sbp_mod
...
type, public, extends(grad_operator_t) :: grad_contra_ah42_sbp_t
...
contains
    procedure, public :: calc_grad => calc_grad_ah42_sbp
end type grad_contra_ah42_sbp_t

contains

subroutine calc_grad_ah42_sbp(this, gx, gy, f, domain, multiplier)
    class(grad_contra_ah42_sbp_t), intent(inout) :: this
...
end subroutine calc_grad_ah42_sbp
...
subroutine calc_grad_on_tile(gx, gy, f, mesh, multiplier)
    ...
    do k = ks, ke
        call sbp_diff("x","d42",f%p(f%is:f%ie,f%js:f%je,k),f%is,f%ie,f%js,f%je,mesh%nx+1,is,ie,js,je,Dx)
        call sbp_diff("y","d42",f%p(f%is:f%ie,f%js:f%je,k),f%is,f%ie,f%js,f%je,mesh%ny+1,is,ie,js,je,Dy)
...
end module grad_contra_ah42_sbp_mod
