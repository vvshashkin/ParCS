...
type, extends(laplace_operator_t) :: laplace_no_metric_t
    ...
    type(sbp_operator_t)           :: sbp_d2_op, sbp_d1_op
...
subroutine calc_laplace(this, f1, f, domain)
    ...
    do t = domain%partition%ts, domain%partition%te
        ...
        ! call calc_tile_d2x_d2y(this%d2f_x%tile(t), this%d2f_y%tile(t), f%tile(t),domain%mesh_xy%tile(t))

        call calc_laplace_tile_metric(f1%tile(t), this%d2f_x%tile(t), this%d2f_y%tile(t), &
                                      this%d2f_xy%tile(t), this%d2f_yx%tile(t), this%d1f_x%tile(t), this%d1f_y%tile(t), &
                                f%tile(t), &
                                this%sbp_d1_op, this%sbp_d2_op, domain%mesh_xy%tile(t), domain%mesh_xy%scale)
...
subroutine calc_laplace_tile2(fout, d2f_x, d2f_y, f, sbp_d2_op, mesh_xy, scale)
    ...
    do k = mesh_xy%ks, mesh_xy%ke
        ...
        if (mesh_xy%is==1) then
            do j = mesh_xy%js, mesh_xy%je
                d2f_x%p(1,j,k) = (-2*f%p(1,j,k)+2*f%p(2,j,k))
            end do
        end if
        do j = mesh_xy%js, mesh_xy%je
            do i = max(mesh_xy%is,2), min(mesh_xy%ie,mesh_xy%nx-1)
                d2f_x%p(i,j,k) = (f%p(i+1,j,k)-2*f%p(i,j,k)+f%p(i-1,j,k))
            end do
        end do
        if (mesh_xy%ie==mesh_xy%nx) then
            do j = mesh_xy%js, mesh_xy%je
                d2f_x%p(mesh_xy%nx,j,k) = (2*f%p(mesh_xy%nx-1,j,k)-2*f%p(mesh_xy%nx,j,k))
            end do
        end if

        if (mesh_xy%js==1) then
            do i = mesh_xy%is, mesh_xy%ie
                d2f_y%p(i,1,k) = (-2*f%p(i,1,k)+2*f%p(i,2,k))
            end do
        end if
        do j = max(mesh_xy%js,2), min(mesh_xy%je,mesh_xy%ny-1)
            do i = mesh_xy%is, mesh_xy%ie
                d2f_y%p(i,j,k) = (f%p(i,j+1,k)-2*f%p(i,j,k)+f%p(i,j-1,k))
            end do
        end do
        if (mesh_xy%je==mesh_xy%ny) then
            do i = mesh_xy%is, mesh_xy%ie
                d2f_y%p(i,mesh_xy%ny, k) = (2*f%p(i,mesh_xy%ny-1,k)-2*f%p(i,mesh_xy%ny,k))
            end do
        end if
...
end subroutine calc_laplace_tile2

subroutine calc_tile_d2x_d2y(d2f_x, d2f_y, f, mesh_xy)
    ...
    !result
    type(tile_field_t), intent(inout) :: d2f_x, d2f_y
    !input
    type(tile_field_t),   intent(in) :: f
    type(tile_mesh_t),    intent(in) :: mesh_xy

    integer(kind=4) :: i, j, k
    real(kind=8)    :: bm, bc, bp, cm, cc, cp, cmm, cpp
...
end subroutine calc_tile_d2x_d2y
...
subroutine calc_laplace_tile_metric(fout,d2f_x, d2f_y, d2f_xy, d2f_yx, d1f_x, d1f_y, f, sbp_d1_op, sbp_d2_op, mesh_xy, scale)
    ...
    !input
    type(tile_field_t),   intent(in) :: f
    type(sbp_operator_t), intent(in) :: sbp_d1_op, sbp_d2_op
    type(tile_mesh_t),    intent(in) :: mesh_xy
    real(kind=8),         intent(in) :: scale
      ...
      if (mesh_xy%is==1) then
          do j = mesh_xy%js, mesh_xy%je
              ...
              penalty = (( 3*f%p(0,j,k) -4*f%p(-1,j,k) + f%p(-2,j,k)) &
                        +( 3*f%p(1,j,k) -4*f%p (2,j,k) + f%p( 3,j,k)))*bc &
                        + 2.0_8*(-2.0_8*d1f_y%p(1,j,k))

              ! fout%p(1,j,k) = fout%p(1,j,k) -0.5_8*penalty
      ...
      if (mesh_xy%ie==mesh_xy%nx) then
          do j = mesh_xy%js, mesh_xy%je
              ...
              penalty = ( ( 3*f%p(mesh_xy%nx  ,j,k)-4*f%p(mesh_xy%nx-1,j,k) + f%p(mesh_xy%nx-2,j,k)) &
                         +( 3*f%p(mesh_xy%nx+1,j,k)-4*f%p(mesh_xy%nx+2,j,k) + f%p(mesh_xy%nx+3,j,k)))*bc &
      ...
      if (mesh_xy%js==1) then
          do i = mesh_xy%is, mesh_xy%ie
              ...
              penalty = ( ( 3*f%p(i,0,k) -4*f%p(i,-1,k) + f%p(i,-2,k)) &
                         +( 3*f%p(i,1,k) -4*f%p(i, 2,k) + f%p(i, 3,k)))*bc &
      ...
      if (mesh_xy%je==mesh_xy%ny) then
          do i = mesh_xy%is, mesh_xy%ie
              ...
              penalty = (( 3*f%p(i,mesh_xy%ny  ,k) -4*f%p(i,mesh_xy%ny-1,k) + f%p(i,mesh_xy%ny-2,k)) &
                        +( 3*f%p(i,mesh_xy%ny+1,k) -4*f%p(i,mesh_xy%ny+2,k) + f%p(i,mesh_xy%ny+3,k)))*bc &
...
end subroutine calc_laplace_tile_metric
! subroutine calc_laplace_tile_metric(fout,d2f_x, d2f_y, d2f_xy, d2f_yx, d1f_x, d1f_y, f, sbp_d1_op, sbp_d2_op, mesh_xy, scale)
!
!     use grid_field_mod, only : grid_field_t, tile_field_t
!     use mesh_mod,       only : tile_mesh_t
...
!
!                 penalty = (( 3*f%p(0,j,k) -4*f%p(-1,j,k) + f%p(-2,j,k)) &
!                           +( 3*f%p(1,j,k) -4*f%p (2,j,k) + f%p( 3,j,k)))
!                 d2f_x%p(1,j,k) = (cc*f%p(1,j,k)+cp*f%p(2,j,k)+cpp*f%p(3,j,k)) - 0.5*bc*penalty
!
!             end do
!         end if
!         do j = mesh_xy%js, mesh_xy%je
...
!
!                 penalty = ( ( 3*f%p(mesh_xy%nx  ,j,k)-4*f%p(mesh_xy%nx-1,j,k) + f%p(mesh_xy%nx-2,j,k)) &
!                            +( 3*f%p(mesh_xy%nx+1,j,k)-4*f%p(mesh_xy%nx+2,j,k) + f%p(mesh_xy%nx+3,j,k)))
!                 d2f_x%p(mesh_xy%nx,j,k) = (cmm*f%p(mesh_xy%nx-2,j,k)+cm*f%p(mesh_xy%nx-1,j,k)+cc*f%p(mesh_xy%nx,j,k)) - 0.5*bc*penalty
!             end do
!         end if
!
!         if (mesh_xy%js==1) then
...
!
!                 penalty = ( ( 3*f%p(i,0,k) -4*f%p(i,-1,k) + f%p(i,-2,k)) &
!                            +( 3*f%p(i,1,k) -4*f%p(i, 2,k) + f%p(i, 3,k)))
!                 d2f_y%p(i,1,k) = (cc*f%p(i,1,k)+cp*f%p(i,2,k)+cpp*f%p(i,3,k)) - bc*0.5*penalty
!             end do
!         end if
!         do j = max(mesh_xy%js,2), min(mesh_xy%je,mesh_xy%ny-1)
!             do i = mesh_xy%is, mesh_xy%ie
...
!
!                 penalty = (( 3*f%p(i,mesh_xy%ny  ,k) -4*f%p(i,mesh_xy%ny-1,k) + f%p(i,mesh_xy%ny-2,k)) &
!                           +( 3*f%p(i,mesh_xy%ny+1,k) -4*f%p(i,mesh_xy%ny+2,k) + f%p(i,mesh_xy%ny+3,k)))
!                 d2f_y%p(i,mesh_xy%ny, k) = (cmm*f%p(i,mesh_xy%ny-2,k)+cm*f%p(i,mesh_xy%ny-1,k)+cc*f%p(i,mesh_xy%ny,k))-0.5*bc*penalty
!
!             end do
!         end if
!
!     end do
    ...
    ! call fout%update(1.0_8/mesh_xy%hx/mesh_xy%hy*scale**(-2), d2f_xy, &
    !                  1.0_8/mesh_xy%hx/mesh_xy%hy*scale**(-2), d2f_yx, mesh_xy)
