...
subroutine calc_grad_contra_a2(this, gx, gy, f, domain, multiplier)
    ...
    do t = domain%partition%ts, domain%partition%te
        call calc_grad_on_tile(gx%tile(t), gy%tile(t), f%tile(t), domain%mesh_p%tile(t), multiplier)
    end do
    ...
    do k = ks, ke
        do j = js, je
            do i = is, ie
                ...
                gx%p(i,j,k) = (mesh%Qi(1,i,j)*fdx + mesh%Qi(2,i,j)*fdy)*mult_loc
                gy%p(i,j,k) = (mesh%Qi(3,i,j)*fdy + mesh%Qi(2,i,j)*fdx)*mult_loc
            end do
        end do
    end do
