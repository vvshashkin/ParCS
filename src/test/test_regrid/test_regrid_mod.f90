...
subroutine test_regrid_vec(staggering,components_type)
    ...
    type(domain_t)                   :: domain
    integer(kind=4),     parameter   :: N=64, nz=3
    integer(kind=4),     parameter   :: Nlon = 4*N, Nlat=2*N+1
    ...
    call create_domain(domain, "cube", staggering, N, nz)
    call create_grid_field(u,0,0,domain%mesh_p)
    call create_grid_field(v,0,0,domain%mesh_p)
    ...
    ! open(117,file="test.dat",access="direct",recl=Nlon*Nlat*nz)
    ! write(117,rec=1) real(utest,4)
    ! write(117,rec=2) real(vtest,4)
    ! write(117,rec=3) real(utest_cub,4)
    ! write(117,rec=4) real(vtest_cub,4)
    ! write(117,rec=5) real(utrue,4)
    ! write(117,rec=6) real(vtrue,4)
    ! close(117)
