    ...
    do t = ts, te
        ...
        do j = js - halo_width, je + halo_width
            ...
            do i = is - halo_width, ie + halo_width
                ...
                mesh%tile(t)%acov(1:3,i,j) = ecs_proto2face(xyz, partition%tile(t)%panel_number)
                xyz = ecs_bcov_proto(alpha,beta)
                mesh%tile(t)%bcov(1:3,i,j) = ecs_proto2face(xyz, partition%tile(t)%panel_number)
                xyz = ecs_actv_proto(alpha,beta)
                mesh%tile(t)%actv(1:3,i,j) = ecs_proto2face(xyz, partition%tile(t)%panel_number)
                xyz = ecs_bctv_proto(alpha,beta)
                mesh%tile(t)%bctv(1:3,i,j) = ecs_proto2face(xyz, partition%tile(t)%panel_number)
